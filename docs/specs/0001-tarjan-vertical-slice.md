...
2. Tarjan's Semantic Model contains stable vertex labels, directed edges, discovery indices, low-link values, on-stack membership, the Tarjan stack, the independent recursive DFS call stack, completed components, the current vertex, the active edge, and the current algorithm phase. Each frame owns a snapshot so stepping backward cannot mutate a later frame.
